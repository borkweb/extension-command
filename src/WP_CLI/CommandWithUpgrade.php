				...
				if ( $is_remote && 'github.com' === $this->parse_url_host_component( $slug, PHP_URL_HOST )
					...
					$filter = function( $source, $remote_source, $upgrader ) use ( $slug, $assoc_args ) {
						...
						if ( $GLOBALS['wp_filesystem']->move( $source, $slug_dir ) ) {
							WP_CLI::log( sprintf( "Renamed Github-based project from '%s' to '%s'.", Utils\basename( $source ), $slug_dir ) );
							return $slug_dir;
						}
					...
					$filter = function( $source, $remote_source, $upgrader ) use ( $slug,  $assoc_args ) {
						...
						if ( $GLOBALS['wp_filesystem']->move( $source, $slug_dir ) ) {
							WP_CLI::log( sprintf( "Renamed project from '%s' to '%s'.", Utils\basename( $source ), $slug_dir ) );
							return $slug_dir;
						}
